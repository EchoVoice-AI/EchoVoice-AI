# 🚀 **EchoVoice**: Customizing the app

[📺 Watch: EchoVoice Intro: Customer Personalization Orchestrator](https://youtu.be/QrJkjsi4aQc?si=N_YnDJJxIsPNsMks)
...
- [🚀 **EchoVoice**: Customizing the app](#-echovoice-customizing-the-app)
  ...
  - [Customizing the backend](#customizing-the-backend)
    - [Chat/Ask approaches](#chatask-approaches)
      - [Chat approach](#chat-approach)
        - [Chat with multimodal feature](#chat-with-multimodal-feature)
      - [Ask approach](#ask-approach)
      - [Ask with multimodal feature](#ask-with-multimodal-feature)
      - [Making settings overrides permanent](#making-settings-overrides-permanent)
  - [Improving answer quality](#improving-answer-quality)
    ...
    - [Improving Azure AI Search results](#improving-azure-ai-search-results)
      - [Configuring parameters in the app](#configuring-parameters-in-the-app)
...
The frontend is built using [Next.js](https://nextjs.org/) (React) and [Material UI components](https://materialui.com/). The frontend components are stored in the `frontend/src` folder. To modify the page title, header text, example questions, and other UI elements, you can customize the `frontend/src/` folder for different languages (English is the default). The primary strings and labels used throughout the application are defined within these files.

## Customizing the backend

The backend is built using [FastAPI](https://fastapi.tiangolo.com/), a Python framework for building high-performance APIs. The backend code is stored in the `backend` folder. The frontend and backend communicate over HTTP using JSON or streamed NDJSON responses. Learn more in the [HTTP Protocol guide](http_protocol.md).
