# Evaluating Customer Personalization Orchestrator  quality


[📺 Watch: Retrieval Deep Dive — Evaluating retrieval answer quality](https://www.youtube.com/watch?v=lyCLu53fb3g)

Follow these steps to evaluate the quality of the answers generated by the EchoVoice retrieval flow.

- [Evaluating Customer Personalization Orchestrator  quality](#evaluating-customer-personalization-orchestrator--quality)
  ...
  - [Run bulk evaluation on a PR](#run-bulk-evaluation-on-a-pr)
    - [Evaluate multimodal retrieval answers](#evaluate-multimodal-retrieval-answers)
...
- `kgfile`: An existing knowledge base (RAGAS) JSON file, which is usually `ground_truth_kg.json`. You may want to specify this if you already created a knowledge base and just want to tweak the question generation steps.
...
Review the configuration in `evals/evaluate_config.json` to ensure that everything is correctly setup. You may want to adjust the metrics used. See [the ai-rag-chat-evaluator README (evaluator)](https://github.com/Azure-Samples/ai-rag-chat-evaluator) for more information on the available metrics.
...
## Evaluate multimodal retrieval answers

The repository also includes an `evaluate_config_multimodal.json` file specifically for evaluating multimodal retrieval answers. This configuration uses a different ground truth file, `ground_truth_multimodal.jsonl`, which includes questions based off the sample data that require both text and image sources to answer.

Note that the "groundedness" evaluator is not reliable for multimodal retrieval, since it does not currently incorporate the image sources. We still include it in the metrics, but the more reliable metrics are "relevance" and "citations matched".
