<!--
---
name: EchoVoice with document security
description: This guide demonstrates how to add an optional login and document level access control system to an EchoVoice deployment for your domain data. This system can be used to restrict access to indexed data to specific users.
languages:
- python
- typescript
- bicep
...
- [🚀 **EchoVoice: Customer Personalization Orchestrator**: Setting up optional login and document level access control](#-echovoice-customer-personalization-orchestrator-setting-up-optional-login-and-document-level-access-control)
...
- Select **New Registration**.
  - In the **Name** section, enter a meaningful application name. This name will be displayed to users of the app, for example `EchoVoice API`.
...
- Select **Expose an API** in the left hand menu. The server app works by using the [On Behalf Of Flow](https://learn.microsoft.com/entra/identity-platform/v2-oauth2-on-behalf-of-flow#protocol-diagram), which requires the server app to expose at least 1 API.
  ...
  - Fill in the values as indicated:
    ...
    - For **Admin consent display name**, type **Access EchoVoice API**.
    - For **Admin consent description**, type **Allows the app to access the EchoVoice API as the signed-in user.**.
    - For **User consent display name**, type **Access EchoVoice API**.
    - For **User consent description**, type **Allow the app to access the EchoVoice API on your behalf**.
...
- Select **New Registration**.
  ...
  - Under `Redirect URI (optional)` section, select `Single-page application (SPA)` in the combo-box and enter the following redirect URI:
    - If you are running the sample locally, add the endpoints `http://localhost:8000/redirect` and `http://localhost:8082/redirect`
    ...
    - If you are running the sample from Github Codespaces, add the Codespaces endpoint: `https://<your-codespace>-8000.app.github.dev/redirect`
- Select **Register** to create the application
- In the app's registration screen, find the **Application (client) ID**.
...
- In the left hand menu, select **API permissions**. You will add permission to access the **access_as_user** API on the server app. This permission is required for the [On Behalf Of Flow](https://learn.microsoft.com/entra/identity-platform/v2-oauth2-on-behalf-of-flow#protocol-diagram) to work.
  ...
  - In the list of applications, select your server application **EchoVoice API**
...
Get an access token that can be used for calling the EchoVoice API using the following code:
...
- `AZURE_ENABLE_UNAUTHENTICATED_ACCESS`: Allows unauthenticated users to access the EchoVoice app. If `AZURE_ENFORCE_ACCESS_CONTROL` is enabled, unauthenticated users cannot search on documents.
